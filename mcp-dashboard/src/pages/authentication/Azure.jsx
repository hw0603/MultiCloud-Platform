...
const ModalComponentCreateProvider = () => {
    ...
    const createProvider = () => {
        ...
        const provider_subID = document.getElementById("provider_subID").value;
        const provider_clientID = document.getElementById("provider_clientID").value;
        const provider_clientSecret = document.getElementById("provider_clientSecret").value;
        const provider_tenantID = document.getElementById("provider_tenantID").value;

        if (provider_team && provider_environment && provider_subID && provider_clientID && provider_clientSecret && provider_tenantID) {

            axios({
                method: "POST",
                url: `${base_url}/api/v1/azure`,
                headers: {
                ...
                data: {
                    ...
                    subscription_id: provider_subID,
                    client_id: provider_clientID,
                    client_secret: provider_clientSecret,
                    tenant_id: provider_tenantID,
                }
            })
                .then((response) => {
    ...
    return (
        <>
            <div className="mt-6 flex flex-col gap-4">
                ...
                <div>
                    <p className="mb-1">Subscription ID</p>
                    <input id="provider_subID" type="text" className="border-2 p-2 rounded-xl w-full" />
                </div>
                <div>
                    <p className="mb-1">Client ID</p>
                    <input id="provider_clientID" type="text" className="border-2 p-2 rounded-xl w-full" />
                </div>
                <div>
                    <p className="mb-1">Client Secret</p>
                    <input id="provider_clientSecret" type="text" className="border-2 p-2 rounded-xl w-full" />
                </div>
                <div>
                    <p className="mb-1">Tenant ID</p>
                    <input id="provider_tenantID" type="text" className="border-2 p-2 rounded-xl w-full" />
                </div>
            </div>
            <div className="mt-10">
                <Button
